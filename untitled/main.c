...
void remover(Node **head, Node **tail, Node *pont);
Node *k(Node *head, Node *tail, int k);
Node *m(Node *head, Node *tail, int m);
...
int main(void) {
  ...
  while (1) {
    ...
    while (N > 0) {
      pontk = k(head, tail, k_value);
      pontm = m(head, tail, m_value);
      remover(&head, &tail, pontk);
      remover(&head, &tail, pontm);
      N -= 2;
    }
